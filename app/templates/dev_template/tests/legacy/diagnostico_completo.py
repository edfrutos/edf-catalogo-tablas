        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if not error_encontrado:
                    logger.info(
                        f"✅ Acceso a {nombre} exitoso (código 200, título: {titulo})"
                    )
                    resultados[nombre] = {
                        "status": "ok",
                        "codigo": response.status_code,
                ...
                # Redirección
                logger.warning(
                    f"⚠️ Acceso a {nombre} redirige a: {response.headers.get('Location')}"
                )
                resultados[nombre] = {
                    "status": "redirect",
                    "codigo": response.status_code,
                ...
                logger.error(
                    f"❌ Acceso a {nombre} fallido: código {response.status_code}"
                )
                resultados[nombre] = {"status": "error", "codigo": response.status_code}

        except Exception as e:
            logger.error(f"❌ Error al verificar acceso a {nombre}: {str(e)}")
            resultados[nombre] = {"status": "error", "mensaje": str(e)}
...
def verificar_formularios():
    ...
    formularios = {
        "Crear catálogo": {"url": f"{BASE_URL}/catalogs/create", "method": "GET"},
        "Editar catálogo": {"url": f"{BASE_URL}/tables", "method": "GET"},
        "Importar catálogo": {"url": f"{BASE_URL}/tables", "method": "GET"},
    }
    ...
    for nombre, info in formularios.items():
        try:
            ...
            if response.status_code == 200:
                ...
                if forms:
                    logger.info(
                        f"✅ Formulario {nombre} encontrado ({len(forms)} formularios en la página)"
                    )
                    resultados[nombre] = {
                        "status": "ok",
                        "codigo": response.status_code,
                    ...
                    logger.warning(
                        f"⚠️ No se encontraron formularios en la página {nombre}"
                    )
                    resultados[nombre] = {
                        "status": "warning",
                        "codigo": response.status_code,
                ...
                # Redirección
                logger.warning(
                    f"⚠️ Acceso a formulario {nombre} redirige a: {response.headers.get('Location')}"
                )
                resultados[nombre] = {
                    "status": "redirect",
                    "codigo": response.status_code,
                ...
                logger.error(
                    f"❌ Acceso a formulario {nombre} fallido: código {response.status_code}"
                )
                resultados[nombre] = {"status": "error", "codigo": response.status_code}
    ...
    for nombre, url in paginas.items():
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(
                    f"✅ Página {nombre}: {len(enlaces)} enlaces y {len(botones)} botones encontrados"
                )
                ...
                for enlace in enlaces:
                    ...
                    if href and texto:
                        enlaces_info.append({"href": href, "texto": texto})
                ...
                for boton in botones:
                    ...
                    if texto:
                        botones_info.append({"tipo": tipo, "texto": texto})
            ...
            logger.error(
                f"❌ Error al verificar botones y enlaces en {nombre}: {str(e)}"
            )
            resultados[nombre] = {"status": "error", "mensaje": str(e)}
...
def verificar_estructura_mongodb():
    ...
    if client is None:
        logger.error(
            "No se pudo conectar a MongoDB. Abortando verificación de estructura."
        )
        return None
    ...
    try:
        ...
        # Verificar usuarios
        usuarios = list(
            db.users.find({}, {"_id": 1, "username": 1, "email": 1, "role": 1})
        )
...
def verificar_permisos_rutas():
    ...
    rutas_admin = ["/admin/", "/admin/users", "/admin/catalogs"]

    rutas_usuario = ["/admin/maintenance/dashboard_user", "/tables", "/catalogs/"]

    resultados = {"admin": {}, "usuario": {}}
    ...
    for ruta in rutas_admin:
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning(
                    f"⚠️ Ruta de administrador {ruta} redirige a: {response.headers.get('Location')}"
                )
                resultados["admin"][ruta] = {
                    "status": "redirect",
                    "codigo": response.status_code,
                ...
                logger.error(
                    f"❌ Acceso a ruta de administrador {ruta} fallido: código {response.status_code}"
                )
                resultados["admin"][ruta] = {
                    "status": "error",
                    "codigo": response.status_code,
            ...
            logger.error(
                f"❌ Error al verificar ruta de administrador {ruta}: {str(e)}"
            )
            resultados["admin"][ruta] = {"status": "error", "mensaje": str(e)}
    ...
    for ruta in rutas_usuario:
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning(
                    f"⚠️ Ruta de usuario {ruta} redirige a: {response.headers.get('Location')}"
                )
                resultados["usuario"][ruta] = {
                    "status": "redirect",
                    "codigo": response.status_code,
                ...
                logger.error(
                    f"❌ Acceso a ruta de usuario {ruta} fallido: código {response.status_code}"
                )
                resultados["usuario"][ruta] = {
                    "status": "error",
                    "codigo": response.status_code,
            ...
            resultados["usuario"][ruta] = {"status": "error", "mensaje": str(e)}
...
def verificar_funcionalidad_tablas():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if forms:
                logger.info(
                    f"✅ Página de tablas accesible ({len(forms)} formularios encontrados)"
                )
                ...
                if form_crear:
                    ...
                    if boton_envio:
                        ...
                        logger.warning(
                            "⚠️ No se encontró botón de envío en el formulario"
                        )
            ...
            # Redirección
            logger.warning(
                f"⚠️ Acceso a página de tablas redirige a: {response.headers.get('Location')}"
            )
            return {
                "status": "redirect",
                "codigo": response.status_code,
            ...
            logger.error(
                f"❌ Acceso a página de tablas fallido: código {response.status_code}"
            )
            return {"status": "error", "codigo": response.status_code}
...
def analizar_problemas_y_soluciones():
    ...
    for nombre, info in rutas_principales.items():
        if info.get("status") != "ok":
            problemas.append(
                f"Problema de acceso a {nombre}: {info.get('mensaje', 'Error desconocido')}"
            )

            if info.get("status") == "redirect":
                soluciones.append(
                    f"Corregir redirección de {nombre} a {info.get('location')}"
                )
            else:
                soluciones.append(
                    f"Verificar permisos y configuración de la ruta {nombre}"
                )
    ...
    for nombre, info in formularios.items():
        if info.get("status") != "ok":
            problemas.append(
                f"Problema con formulario {nombre}: {info.get('mensaje', 'Error desconocido')}"
            )

            if info.get("status") == "redirect":
                soluciones.append(
                    f"Corregir redirección del formulario {nombre} a {info.get('location')}"
                )
            else:
    ...
    if funcionalidad_tablas.get("status") != "ok":
        problemas.append(
            f"Problema con la funcionalidad de tablas: {funcionalidad_tablas.get('mensaje', 'Error desconocido')}"
        )

        if funcionalidad_tablas.get("status") == "redirect":
            soluciones.append(
                f"Corregir redirección de la página de tablas a {funcionalidad_tablas.get('location')}"
            )
        else:
