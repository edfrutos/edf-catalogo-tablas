...
def agregar_fila_route(main_bp):
    @main_bp.route("/agregar_fila/<tabla_id>", methods=["GET", "POST"])  # type: ignore
    def agregar_fila(tabla_id: str) -> Any:
        # Verificar sesión
        ...
        try:
            # Obtener la tabla
            tabla = current_app.spreadsheets_collection.find_one(  # type: ignore
            ...
            if session.get("role") != "admin" and session.get("username") != tabla.get(
                "owner"
            ):
