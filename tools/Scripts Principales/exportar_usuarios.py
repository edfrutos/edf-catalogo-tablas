...
if not MONGO_URI or not DB_NAME:
    print(
        "[ERROR] Debes definir MONGO_URI y MONGO_DBNAME en tu archivo .env o en las variables de entorno."
    )
    sys.exit(1)

ALLOW_INVALID_CERTS = os.environ.get("MONGO_ALLOW_INVALID_CERTS", "false").lower() in [
    "true",
    "1",
    "yes",
]
...
try:
    ...
    if not ALLOW_INVALID_CERTS:
        print(
            "Si el error es de certificados, puedes probar a ejecutar el script con la variable de entorno MONGO_ALLOW_INVALID_CERTS=true (solo para pruebas)"
        )
    sys.exit(2)
